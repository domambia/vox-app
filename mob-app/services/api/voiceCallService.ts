...
class VoiceCallService {
   ...
   */
  async updateCallStatus(callId: string, data: UpdateCallStatusData): Promise<VoiceCall> {
    const response: AxiosResponse = await apiClient.put(`/calls/${callId}/status`, {
      status: data.status,
    });
    return this.mapCall(response.data?.data ?? response.data);
  }
  ...
  async endCall(callId: string): Promise<VoiceCall> {
    const response: AxiosResponse = await apiClient.post(`/calls/${callId}/end`);
    return this.mapCall(response.data?.data ?? response.data);
  }
   ...
   */
  async getCallHistory(params?: { page?: number; limit?: number }): Promise<PaginatedCallsResponse> {
    ...
    const items = Array.isArray(data?.items) ? data.items : (data?.calls ?? data?.data ?? []);
   ...
   */
  async getCallRoom(callId: string): Promise<{ roomSid?: string; [key: string]: any }> {
    const response: AxiosResponse = await apiClient.get(`/calls/${callId}/room`);
    return response.data?.data ?? response.data ?? {};
  }
