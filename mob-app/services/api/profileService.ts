...
function mapBackendProfile(raw: BackendProfile): Profile {
  ...
  if (voiceBioUrl && !voiceBioUrl.startsWith('http')) {
    ...
    voiceBioUrl = voiceBioUrl.startsWith('/') ? `${base}${voiceBioUrl}` : `${base}/${voiceBioUrl}`;
  }
  return {
...
class ProfileService {
  ...
  async createProfile(data: CreateProfileData): Promise<Profile> {
    const response: AxiosResponse = await apiClient.post('/profile', data);
  ...
  async updateProfile(userId: string, data: UpdateProfileData): Promise<Profile> {
    const response: AxiosResponse = await apiClient.put(`/profile/${userId}`, data);
    const raw = response.data?.data ?? response.data;
    return mapBackendProfile(raw as BackendProfile);
  }
  ...
  async uploadVoiceBio(file: FormData): Promise<{ voiceBioUrl: string }> {
    const response: AxiosResponse = await apiClient.post('/profile/voice-bio', file, {
      headers: {
   ...
   */
  async likeProfile(userId: string): Promise<{ isMatch: boolean; matchId?: string; message?: string }> {
    const response: AxiosResponse = await apiClient.post(`/profile/${userId}/like`);
    const data = response.data?.data ?? response.data;
    return {
