...
class GroupsService {
  ...
  async createGroup(data: CreateGroupData): Promise<Group> {
    const body = {
      ...
      isPublic: data.isPublic ?? (data.isPrivate !== undefined ? !data.isPrivate : true),
    };
  ...
  async listGroups(params?: ListGroupsParams): Promise<PaginatedResponse<Group>> {
    ...
    if (params?.isPrivate !== undefined) queryParams.append('is_private', params.isPrivate.toString());
  ...
  async updateGroup(groupId: string, data: UpdateGroupData): Promise<Group> {
    const response: AxiosResponse = await apiClient.put(`/groups/${groupId}`, data);
    return response.data.data;
  }
   ...
   * Get group members
   */
  async getGroupMembers(params: GetGroupMembersParams): Promise<PaginatedResponse<GroupMember>> {
    const queryParams = new URLSearchParams();
    if (params.page) queryParams.append('page', params.page.toString());
    if (params.limit) queryParams.append('limit', params.limit.toString());
    if (params.role) queryParams.append('role', params.role.toLowerCase());
   ...
   */
  async updateMemberRole(groupId: string, memberId: string, data: UpdateMemberRoleData): Promise<GroupMember> {
    const response: AxiosResponse = await apiClient.put(`/groups/${groupId}/members/${memberId}/role`, data);
    return response.data.data;
  }
   ...
   */
  async getGroupMessages(params: GetGroupMessagesParams): Promise<{ items: GroupMessage[]; pagination: any }> {
    ...
    const response: AxiosResponse = await apiClient.get(url);
    const data = response.data?.data ?? response.data;
    const items = (data?.items ?? data?.messages ?? []).map((m: any) => this.mapGroupMessage(m));
    const pagination = data?.pagination ?? { limit: 50, offset: 0, total: items.length, totalPages: 1 };
   ...
   */
  async sendGroupMessage(groupId: string, content: string, messageType: string = 'TEXT'): Promise<GroupMessage> {
    const response: AxiosResponse = await apiClient.post(`/groups/${groupId}/messages`, {
  ...
  private mapGroupMessage(raw: any): GroupMessage {
    ...
    return {
      ...
      content: m.content ?? '',
      messageType: (m.message_type ?? m.messageType ?? 'TEXT') as GroupMessage['messageType'],
   ...
   */
  async getUserGroups(userId: string, params?: { page?: number; limit?: number }): Promise<{ groups: Group[] }> {
    const queryParams = new URLSearchParams();
    if (params?.page) queryParams.append('page', params.page.toString());
    if (params?.limit) queryParams.append('limit', params.limit.toString());
